...
- Тесты, проверяющие взаимодействие нескольких компонентов системы (база данных, очередь сообщений, внешние сервисы и т.д.).
- Они находятся между unit‑тестами (изолированные) и end‑to‑end тестами (полный стек).
- Цель – убедиться, что компоненты работают корректно в реальных условиях.

### 2. Общие принципы написания
| Принцип | Что это означает | Как реализовать |
|---|---|---|
| **Изолировать тест‑среду** | Каждый запуск должен происходить в чистом окружении. | Использовать Testcontainers, Docker‑compose или мок‑сервера.
| **Проверять реальные сценарии** | Тесты должны воспроизводить реальный путь данных. | Создавать реальные сообщения, запросы и проверять ответы.
| **Минимум внешних зависимостей** | Чем меньше «живых» сервисов, тем быстрее и надёжнее. | Мокать внешние API через WireMock/httptest.
| **Чёткая проверка** | Тесты должны явно утверждать ожидаемый результат. | `require.Equal`, `assert.Contains` и т.п.
| **Повторяемость** | Тесты должны проходить каждый раз без случайных падений. | Устанавливать фиксированные таймауты, использовать `t.Parallel()` только там, где безопасно.
| **Логирование и трассировка** | При падении важно быстро понять причину. | `t.Log`, `logrus` в тестах, экспортировать логи.
| **Документирование** | Тесты – это живая документация. | Добавлять комментарии и README‑подсказки.

### 3. Инструменты, используемые в проекте
| Инструмент | Назначение | Как используется |
|---|---|---|
| **Testcontainers‑go** (`github.com/testcontainers/testcontainers-go`) | Запуск контейнеров (Kafka, PostgreSQL и др.) в тестах. | `testkafka.Run`, `postgresql.Run`.
| **Testify** (`github.com/stretchr/testify`) | Ассерты и тест‑сьюты. | `require.NoError`, `suite.Suite`.
| **Go testing** (`testing`) | Базовый фреймворк. | `TestXxx`, `t.Run`.
| **WireMock / httptest** | Мок внешних HTTP‑сервисов. | `httptest.NewServer`.
| **Docker Compose** (не в коде, но для CI) | Запуск нескольких сервисов одновременно. |

### 4. Структура тест‑сuites в проекте
1. **SetupSuite / TearDownSuite** – создаём и уничтожаем контейнеры.
2. **TestXxx** – каждый тест проверяет один сценарий.
3. **Таймауты** – задаём `time.After` для ожидания асинхронных событий.
4. **Параллелизм** – если компоненты независимы, можно использовать `t.Parallel()`.

### 5. Пример: тестирование Kafka‑сообщений
```go
func (s *EventsSuite) TestWrite_ProducesMessage() {
    ctx := s.T().Context()

    // 1. Создаём продюсер
    ev, err := events.New(
        producer.WithBrokers(s.brokers...),
        producer.WithTopic(s.topic),
    )
    require.NoError(s.T(), err)

    // 2. Запускаем продюсер и консьюмера
    require.NoError(s.T(), ev.Start(ctx))
    h, err := consumer.New(...)
    require.NoError(s.T(), h.Start(ctx))

    // 3. Отправляем событие
    event := domain.Event{ID: uuid.New(), Type: "test", Data: []byte("{\"ok\":true}")}
    require.NoError(s.T(), ev.Write(ctx, event))

    // 4. Ожидаем подтверждение в консьюмере
    select {
    case <-received:
        // ok
    case <-time.After(10 * time.Second):
        s.T().Fatal("did not receive message in time")
    }
}
```

### 6. Best‑practice из внешних источников (Context7)
- **Параметризованные тесты** – использовать `t.Run` с разными наборами входных данных.
- **Тест‑фичи** – хранить тестовые данные в `fixtures` и загружать через `sqlx`.
- **Таймауты** – использовать `context.WithTimeout` вместо `time.After`.
- **Логирование** – включать `logrus.WithField("test", t.Name())`.
- **CI‑интеграция** – запускать тесты в Docker‑контейнерах, чтобы избежать конфликтов портов.

### 7. Как писать новый интеграционный тест
1. Определите сценарий и необходимые внешние сервисы.
2. В `SetupSuite` запустите нужные контейнеры (Kafka, Postgres и т.д.).
3. В `TearDownSuite` завершите их.
4. Напишите тест, используя `require`/`assert` и таймауты.
5. Добавьте комментарии, объясняющие цель теста.
6. Запустите `go test -tags=integration ./tests/integration/...` и убедитесь, что тест проходит.
